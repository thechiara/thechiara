...
static int
extract_bdisp (unsigned insn, int *invalid __attribute__((unused)))
{
...
static int
extract_jhint (unsigned insn, int *invalid __attribute__((unused)))
{
...
static int
extract_ev6hwjhint (unsigned insn, int *invalid __attribute__((unused)))
{
...
#define MDISP		(LIT + 1)
  ...
  /* The signed "23-bit" aligned displacement of Branch format insns. modified by gaspard  */
 ...
 * 
 */
void chiara_parse_alpha_instruction(unsigned long  instruction) {
	unsigned char opcodebasic = AXP_OP(instruction);
	//PPC_OP
	for(int x = 0;x<alpha_num_opcodes;x++) {

		if(AXP_OP(alpha_opcodes[x].opcode) == opcodebasic) {
													printf("FOUND ALPHA %s \n ",alpha_opcodes[x].name);

			int status = 0;

			while(status!= 4 && alpha_opcodes[x].operands[status] != 0) {
				int error;
				if(*alpha_operands[alpha_opcodes[x].operands[status]].extract != 0) {
					
				  signed long long valuereturned = 	alpha_operands[alpha_opcodes[x].operands[status]].extract(instruction,&error);
						if(error !=1) {
					// We have found our instruction great !
					// CALL CHIARAGLOBAL HERE
					break;
...
void chiara_emul_littleendian_dec(unsigned char *instruction,int size) {
	long statusarray=0;
	long shiftnum=0;
	unsigned long  *instructionbuiled = malloc(32);
	for(;statusarray<size;statusarray++) {
		*instructionbuiled |= instruction[statusarray] << shiftnum;
		if(shiftnum == 24) {
			// compile instrction 
			
			chiara_parse_alpha_instruction(*instructionbuiled);
			free(instructionbuiled);
			instructionbuiled = malloc(32);
			shiftnum = 0;
			} else {
		shiftnum += 8;
	}
